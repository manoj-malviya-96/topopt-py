...
class DirectSolver:
    ...
    __slots__ = ('_free_dofs', '_n_dof', '_displacement', '_use_cholmod')
    ...
    def solve(self, K: SparseMatrix, f: NDArray[np.float64]) -> NDArray[np.float64]:
        ...
        if self._use_cholmod:
            u_free = cholmod_cholesky(K_free)(f_free)
        else:
...
def compute_sensitivity(elem_dof_indices: NDArray[np.int64], ke: NDArray[np.float64],
                        displacement: NDArray[np.float64], penal: float,
                        E: float, E_min: float) -> NDArray[np.float64]:
    ...
    return -penal * (E - E_min) * strain_energy
